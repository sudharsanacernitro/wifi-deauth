...
while True:
    ...
    for i in range(0,len(bssid)):
        if(int(channel[i])==j):
            if(kl%1==0):
                os.system(f"gnome-terminal --command 'python3 p.py -f {bssid[i]} -l {channel[i]}'")
            u=0
        if(j==16):
            j=1
    j=j+1
    ...
    if(u==0):
        ...
        #                os.system(f"timeout 10s gnome-terminal --tab -- bash -c ' python3 p.py -f {bssid[i]} -l {channel[i]}; exec bash'")
